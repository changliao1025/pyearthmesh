...
from pyearth.gis.gdal.gdal_vector_format_support import get_vector_driver_from_filename #future support for multiple vector formats
...
def convert_cubicspheremesh_to_pyflowline_mesh(
    ...
    for i in range(nCell):
        ...
        if iFlag == True:
            pcubicsphere = convert_gcs_coordinates_to_meshcell(
                8, dLongitude_center, dLatitude_center, aCoords_gcs
            )
...
def create_cubicsphere_mesh(
    ...
    aCubicsphere = convert_cubicspheremesh_to_pyflowline_mesh(
        iFlag_global_in,
        sFilename_csmesh,
        sFilename_output_in,
