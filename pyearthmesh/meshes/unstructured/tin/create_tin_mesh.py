            ...
            for i in range(nVertex):
                ...
                if iFlag_global == 1:
                    ...
                    if pPolygon.Within(pBoundary):
                        ...
                        dLon_min, dLon_max = np.min(aCoords_gcs[:, 0]), np.max(
                            aCoords_gcs[:, 0]
                        )
                        if (
                ...
                if iFlag == True:
                    dLongitude_center = float(
                        np.mean(aCoords_gcs[0:nVertex, 0])
                    )  # these are not the actual center, see circumcenter function
                    dLatitude_center = float(np.mean(aCoords_gcs[0:nVertex, 1]))
                    pTin = convert_gcs_coordinates_to_meshcell(
                        7, dLongitude_center, dLatitude_center, aCoords_gcs
                    )
