...
from pyearthmesh.utility.mpas_tools.mpasmsh import (
    jigsaw_mesh_to_netcdf,
    inject_edge_tags,
    subtract_critical_passages,
...
def saveesm(
    ...
    if geom.mshID.lower() == "ellipsoid-mesh":
        ...
        sFilename_triangles = os.path.join(
            sWorkspace_jigsaw_out, "tmp", "mesh_triangles.nc"
        )
        jigsaw_mesh_to_netcdf(
            mesh=mesh,
            on_sphere=True,
    ...
    if geom.mshID.lower() == "euclidean-mesh":
        ...
        jigsaw_mesh_to_netcdf(
            mesh=mesh, on_sphere=False, output_name=sFilename_triangles
        )
    ...
    if iFlag_vtk == 1:
        ...
        sFilename_base_mesh_vtk = os.path.join(
            sWorkspace_jigsaw_out, "out", "base_mesh_vtk"
        )
        args = [
            sPath_executable,
            "--ignore_time",
    ...
    if sFilename_land_ocean_mask_in is None:
        sFilename_land_coverage = os.path.join(
            sWorkspace_jigsaw_out, "tmp", "land_coverage.geojson"
        )
        _land_mask_from_geojson(
            sFilename_base_mesh, sFilename_land_coverage, sFilename_land_mask
        )
    else:
        sFilename_land_coverage = os.path.join(
            sWorkspace_jigsaw_out, "tmp", "land_coverage.geojson"
    ...
    iFlag_old = 0
    sFilename_culled_mesh = os.path.join(
        sWorkspace_jigsaw_out, "out", "culled_graph.info"
    )
    sFilename_invert_mesh = os.path.join(
        sWorkspace_jigsaw_out, "out", "invert_graph.info"
    )
    if iFlag_old == 1:
        dsCulledMesh = cull(
            dsBaseMesh, dsMask=dsLandMask, graphInfoFileName=sFilename_culled_mesh
        )

        dsInvertMesh = cull(
            dsBaseMesh, dsInverse=dsLandMask, graphInfoFileName=sFilename_invert_mesh
        )
    else:
    ...
    write_netcdf(dsCulledMesh, sFilename_culled_mesh, format=netcdfFormat)

    sFilename_invert_mesh = os.path.join(sWorkspace_jigsaw_out, "out", "invert_mesh.nc")
    write_netcdf(dsInvertMesh, sFilename_invert_mesh, format=netcdfFormat)

    if iFlag_vtk == 1:
        sFilename_culled_mesh_vtk = os.path.join(
            sWorkspace_jigsaw_out, "out", "culled_mesh_vtk"
        )
        args = [
            sPath_executable,
            "--ignore_time",
            ...
            "allOnCells",
            "-f",
            sFilename_culled_mesh,
            "-o",
            sFilename_culled_mesh_vtk,
        ]
        ...
        sFilename_invert_mesh_vtk = os.path.join(
            sWorkspace_jigsaw_out, "out", "invert_mesh_vtk"
        )
        args = [
            sPath_executable,
            "--ignore_time",
            ...
            "allOnCells",
            "-f",
            sFilename_invert_mesh,
            "-o",
            sFilename_invert_mesh_vtk,
        ]
        print("running", " ".join(args))
        args = [sPython] + args
        subprocess.check_call(args, env=os.environ.copy())
