...
def create_mpas_mesh(
    ...
    if iFlag_save_mesh == 1:
        ...
        if iFlag_use_mesh_dem == 1:
            pLayer.CreateField(
                ogr.FieldDefn("elevation_mean", ogr.OFTReal)
            )  # float type for high resolution
            pLayer.CreateField(
                ogr.FieldDefn("elevation_profile0", ogr.OFTReal)
            )  # float type for high resolution
        else:
            pass
    ...
    def add_cell_into_list(
        ...
        if len(aVertexIndex) == len(aEdgeIndex) and len(aEdgeIndex) == len(
            ...
            pmpas = convert_gcs_attributes_to_meshcell(
                4,
                dLongitude_center,
                dLatitude_center,
                aCoords_gcs,
